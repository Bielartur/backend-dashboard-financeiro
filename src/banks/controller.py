...
async def create_bank(
    ...
    return await service.create_bank(current_user, db, bank)
...
async def get_banks(db: DbSession, current_user: TokenData = Depends(get_current_user)):
    return await service.get_banks(current_user, db)
...
async def get_bank(
    ...
    return await service.get_bank_by_id(current_user, db, bank_id)
...
async def update_bank(
    ...
    return await service.update_bank(current_user, db, bank_id, bank_update)
...
async def delete_bank(db: DbSession, bank_id: UUID, current_user: CurrentAdmin):
    return await service.delete_bank(current_user, db, bank_id)
